...
{
    public class UserOperationClaim : IEntity<UserOperationClaim>
    {
        ...
        public class Repository : EfBaseRepository<UserOperationClaim, ZgnAgvManagerContext>
        {
        }
        public IDataResult<List<UserOperationClaim>> GetAll(Expression<Func<UserOperationClaim, bool>>? filter = null)
        {
            return new SuccessDataResult<List<UserOperationClaim>>(_repository.GetAll(filter), "Listed all data");
        }
        public IDataResult<UserOperationClaim?> Get(Expression<Func<UserOperationClaim, bool>> filter)
        {
            return new SuccessDataResult<UserOperationClaim?>(_repository.Get(filter), "Get filter data");
        }
        ...
        public async Task<IDataResult<List<UserOperationClaim>>> GetAllAsync(Expression<Func<UserOperationClaim, bool>>? filter = null)
        {
            var result = await _repository.GetAllAsync(filter);
            return new SuccessDataResult<List<UserOperationClaim>>(result, "Listed all data");
        }
        public async Task<IDataResult<UserOperationClaim?>> GetAsync(Expression<Func<UserOperationClaim, bool>> filter)
        {
            ...
            return new SuccessDataResult<UserOperationClaim?>(result, "Get filter data");
        }
