...
export class PendingTransactionComponent implements OnInit {
  ...
  constructor(private messagerService:MessagerService,private router:Router) {}
          ...
          onClick(event:any) {
              if (event.target == 'start') {
                ...
                if (selection.length != 1) {
                  self.messagerService.error('Lütfen bir adet işlem seçiniz.');
                  return;
                }
                self.router.navigateByUrl(`/commit-transaction/${selection[0]}`)
